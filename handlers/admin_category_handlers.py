...
@category_router.message(Command('edit_product'))
async def edit_product(message: Message, state: FSMContext):
    await state.clear()
    await message.answer('Please provide the details for the product you want to edit.')


@category_router.message(Command('del_product'))
async def del_product(message: Message, state: FSMContext):
    await state.clear()
    await message.answer('Please specify the product you want to delete.')


@category_router.message(Command('products'))
async def products(message: Message, state: FSMContext):
    products = db.get_products()

    if products:
        products_text = "\n".join([f"{idx + 1}. {product}" for idx, product in enumerate(products)])
        await message.answer(f'Here are your products:\n{products_text}')
    else:
        await message.answer('You have no products.')
